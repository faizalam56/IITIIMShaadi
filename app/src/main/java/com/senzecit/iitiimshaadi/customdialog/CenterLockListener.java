    ...
    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
        ...
        if( !mAutoSet ) {
            ...
            if( newState == RecyclerView.SCROLL_STATE_IDLE ) {
                ...
                CustomListAdapterDialog rAdapter= (CustomListAdapterDialog) recyclerView.getAdapter();
                int posotion=lm.findFirstCompletelyVisibleItemPosition();
                int positionInList = posotion % rAdapter.listData.size();
//                rAdapter.listData.get(positionInList).setName("A");
                rAdapter.listData.get(positionInList).setColor("1");
                System.out.println("onScrolled "+rAdapter.listData.get(positionInList).getName());
